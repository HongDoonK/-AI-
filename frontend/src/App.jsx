...
function getEmotionMeta(policy = {}) {
  const possibility = String(policy.apply_possibility || policy.fit_level || '');
  const score = parseScore(policy.match_score ?? policy.score ?? policy.similarity_score ?? policy.match_score_label);

  if (possibility.includes('높음') || (score !== null && score >= 0.8)) {
    return { emoji: '😄', label: '추천도 높음', className: 'emotion-high' };
  }
  if (possibility.includes('보통') || (score !== null && score >= 0.6)) {
    return { emoji: '🙂', label: '추천도 보통', className: 'emotion-medium' };
  }
  if (possibility.includes('낮음') || (score !== null && score < 0.45)) {
    return { emoji: '😕', label: '추천도 낮음', className: 'emotion-low' };
  }
  if (possibility.includes('확인') || possibility.includes('미정')) {
    return { emoji: '🤔', label: '추가 확인 필요', className: 'emotion-check' };
  }
  return { emoji: '🤔', label: '추가 확인 필요', className: 'emotion-check' };
}
...
function PolicyCard({ policy, index, onOpenChat }) {
  ...
  const emotion = getEmotionMeta(policy);
  return (
    <article className={`policy-card ${emotion.className}`} tabIndex="0" onClick={() => onOpenChat(policy)} onKeyDown={(event) => { if (event.key === 'Enter') onOpenChat(policy); }}>
      <div className="policy-head">
        ...
        <div className="fit-area">
          <span className="emotion-face" aria-label={emotion.label}>{emotion.emoji}</span>
